...
<body class="bg-darker text-white">
    ...
    <div class="font-[sans-serif] max-w-7xl mx-auto h-screen">
        <div class="grid md:grid-cols-2 items-center gap-8 h-full">
            <form method="POST" action="{{ route('login') }}" class="max-w-lg max-md:mx-auto w-full p-12 border-[1px]">
                @csrf
                <!-- Session Status -->
                @if (session('status'))
                ...
                <div class="flex flex-wrap items-center gap-4 justify-between mt-4">
                    ...
                    @if (Route::has('password.request'))
                        <div class="text-sm">
                            <a href="{{ route('password.request') }}" class="text-gray-400 font-semibold hover:text-white transition-colors">
